...
│   └── 本地配置与可重建缓存
├── CLI（MVP，见 development/cli.md）
│   └── 代码目录 `cli/`（bin `agentero`）；path 复用 src-tauri services（不迁 core；无 BYOA）
└── Vault
...
| Bug 修复 | [`bug_fix/`](bug_fix/) | 已修复问题的现象、根因、方案与参考资料（如 [DeepSeek Thinking](bug_fix/deepseek-thinking-body.md)、[ACP 会话恢复](bug_fix/acp-session-resume-id.md)、[IME 组字 Enter 误发送](bug_fix/ime-composition-enter-submit.md)、[远端 ACP PATH](bug_fix/remote-acp-path-ssh.md)）。 |
...
- Zotero Connector 兼容（MVP）：[`backend/connector.md`](backend/connector.md)
