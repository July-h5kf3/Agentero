...
| 业务壳 | `layout/agent-panel`（Chat）、`layout/file-tree`（Vault 树 + 虚拟 Library 节点 + 魔棒）、`layout/papers-library`（catalog 论文表）、`layout/paper-info-panel`（选中论文元信息）、`layout/backlinks-panel`（反链）、`layout/graph-panel`（图谱） |
| 状态列 | ✅ 已装并接线 · 📦 已装未接线 · — 未安装 |
