...
- **PDF 选区 → 对话卡片**：来自 PDF 且带页内几何（`rects` + `paperAbsPath`）的选区，在 **Agent 发送该轮** 时写入 `agent-trace` mark（`comment` = 选中原文，`messages[]` = 用户问题 + 回复；无裁剪图）。页边针 + 浮层对话卡与视觉批注共用；多轮续聊同样回写 mark。Markdown 选区或缺少几何时仍只作 chip、不落盘。
