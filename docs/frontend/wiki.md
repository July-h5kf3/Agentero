...
- **Frontmatter `aliases`**：笔记 YAML 中的别名参与 file 搜索与 resolve（与 Obsidian 一致）。候选主行显示别名；**副行**显示 `alias · vault相对路径`，与仅文件名命中的「副行只显示路径」区分。选中后写入 `[[canonical|alias]]`（`insertText` 仍是可解析目标）。同名 alias 多文件时列出多条，不静默选第一条。
