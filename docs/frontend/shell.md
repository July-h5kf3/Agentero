...
- **右栏**（可选）：Agent / Backlinks / 批注 / **References** / **Figures**（同样 collapsible）。
  ...
  - **Figures**：版面分析后的插图（image+chart）/ 表 / 算法；默认置信度 ≥30%；联图与标题规则见 [pdf-layout-analysis.md](pdf-layout-analysis.md)。实现：`figures-panel.tsx` + `src/lib/pdf/layout/`（内存 store，未落盘）。
