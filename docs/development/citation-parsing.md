...
- PDF 查看器（EmbedPDF / PDFium）未加载页内 Link annotation，文中 citation 点击未接线；`goToPage` 与 destination 解析（`src/lib/pdf/bookmark.ts`）已有基础。
...
优先级（产品决策，**覆盖 pdf-analysis §4.3「网络补全延后」的约束**）：**在线 API 最高 → 本地 bib/bbl 兜底 → 文本层切分最后**。逐层 fallback，取第一个成功的；首版实现 L1 + L2。
...
- `source/**/*.bib`：标准 BibTeX，brace-aware 字段读取（pdf-analysis §4.1 已规划），直接得到 key、title、author、year、venue、doi。
- **`source/**/*.bbl`（pdf-analysis 未覆盖的 delta，且是 arXiv 主力）**：arXiv e-print 里 `.bbl` 出现率远高于 `.bib`。`\bibitem{key} ...` 半结构化：按 `\bibitem` 切分，剥 TeX 命令（`\newblock`、`\emph`、`{}`），raw 文本必存 + 正则尽力提取 year / arXiv id / DOI / URL。
- 主 `.tex` 内联 `\begin{thebibliography}`：与 `.bbl` 同一解析器。
- 正文 `\cite{}` 系列命令扫描建立 key → 文中位置映射（pdf-analysis §4.1 已规划）。
...
- PDF 对参考文献**没有语义结构**，可提取的只有文本层；「直接解析 PDF」拿不到比 liteparse 文本更多的元数据。元数据解析统一走文本层（liteparse word-boxes 输出，见 pdf-analysis §4.2；`PAPER.md` 仅作回退语料）。
...
- **事实来源：`{paper}/source/agentero-cite.json` sidecar**（契约见 pdf-analysis §2.1），可重建、可删除、不碰用户文件——这同时回答「没有 bib 文件时的持久化」：L1 在线结果与 L3 的 raw + 尽力字段都写入 sidecar，重开应用不重解析、不重复请求 API（fingerprint 判断；`source` 字段记 `s2 | crossref | bib | bbl | tex | pdf-text`）。
- 库内匹配（`localMatch`）：DOI → arXiv id → 归一化 title+author+year，只查本地 catalog（pdf-analysis §4.3）。
- **delta：catalog 建 `paper_refs` 索引表**推迟到需要跨论文查询（引用图谱、被引统计）时再评估，避免双写；MVP 单论文场景 sidecar 足够。
- 导出：右键论文提供 **导出 references.bib**（本地 BibTeX 序列化 sidecar 条目），非默认落盘。

## 4. 文中 citation 交互与引用侧栏

交互契约以 pdf-analysis §5.1–5.2（`Paper Content` 侧栏 + PDF overlay）为准，本文补充卡片形态与联动细节：

- **卡片保持简洁**，单卡仅含：
  - `[12]` 编号徽标 + 标题（两行截断；无标题时显示 raw 前两行）；
  - 第二行：首作者 et al. · 年份 · venue；
  - 角标：DOI / arXiv 外链徽标、**已入库**标记（点击打开库内论文）；未入库 hover 出 **导入**（走 `paper_commit` 管线；注意 pdf-analysis §1.2 明确「不**自动**导入」，此处为用户显式点击，不冲突）。
...
- **点击**：文中 citation 点击跳 References 条目（pdf-analysis §5.2）；卡片点击跳第一个 anchor。
- PAPER.md 视图（无 PDF 时的回退）：编辑器装饰插件把 `[12]`、`[3, 7]` 渲染为可点击 token，纯展示装饰、不改写 Markdown 源文本；点击/hover 复用同一事件。

## 5. Agent 集成

pdf-analysis §5.3 已规划 `@` 菜单 Citations 分组与卡片拖拽；本文补充两个 delta：
...
## 6. 分期（与 pdf-analysis P0–P5 的关系）

| 阶段 | 内容 | 对应 |
| --- | --- | --- |
| M1（首版） | **L1 在线（S2 / Crossref）+ L2 本地（含 `.bbl`）** + sidecar 写入 + 库内匹配 + 在线开关设置 | 并入 pdf-analysis P1 |
| M2 | Paper Content 侧栏 Citations 卡片 + hover/click 双向联动 | P3 |
| M3 | Agent：`@` 分组 + 拖拽 + **`#` 编号提及** + prompt 注入 | P4 |
| M4 | L3 文本层切分 + Crossref raw string 富化 + references.bib 导出 + 未入库一键导入 | P2 / 后续版本 |
...
- author-year 引用样式（`(Smith et al., 2020)`）的文中匹配难于数字编号，首版允许 `unresolved` 降级（与 pdf-analysis 验收标准一致）。
