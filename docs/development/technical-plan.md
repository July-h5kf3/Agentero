...
| 模块 | 路径 | 说明 |
|---|---|---|
| 可伸缩面板 | `react-resizable-panels`（`Group` / `Panel` / `Separator`） | v4 API；封装见 `src/components/layout/shell/resizable.tsx`；左右侧栏 **collapsible 常驻** + `preserve-pixel-size` |
| 侧边栏文件树 | `src/components/layout/sidebar/file-tree.tsx` | 包装 **AI Elements** `FileTree`；右键 / 快捷键 |
