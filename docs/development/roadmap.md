...
- **Zotero Connector 兼容服务**（方案一，**MVP 已落地**）：Host 在 `127.0.0.1:23119` 兼容官方浏览器扩展保存协议 → 当前 Vault；与 Zotero 桌面端端口互斥、默认关；见 [`../backend/connector.md`](../backend/connector.md)。`saveAttachment` 二进制与 cookies 仍待。
