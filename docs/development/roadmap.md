...
- [x] 树内联新建文件 / 文件夹。
- [x] 文件树：**Finder 显示**（`⌥⌘R`）；**终端打开**（`⌥⌘T`）；**多选 + 拖拽移动**；**删除**走回收站（`path_trash`，无确认 / 无 Undo toast；Library 下虚拟节点 `agentero:trash` → 中间栏浏览 / 恢复 / 清空；`papers/` 快照 catalog）；**Paper 行标签**默认标题 · 作者（`paperTreeLabelMode`，设置 → 通用）。
