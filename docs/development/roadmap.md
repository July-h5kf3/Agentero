...
| V0.4 双链、反链与图谱 | ✅ 基本完成 | 语义解析、标题/block 精确跳转、入/出链、Graph、Live Preview、`[[` 文件/alias/标题/block 候选，以及 Markdown/图片/PDF 的 `![[...]]` 只读嵌入已落地；**链接感知的文件/目录/paper 移动**、可信本地外部 rename repair、**显式标题重命名事务**与 Host SQLite 派生 snapshot 也已落地。 |
...
| Release CI | ✅ 完成 | push `v*` tag 时构建 macOS/Linux/Windows Tauri 安装包并上传草稿 Release；macOS 可选 Developer ID + notarytool（secrets 齐后生效）。 |
...
**设计文档**：`docs/backend/wikilinks.md`（Obsidian 兼容模型、开源选型、Phase A–F）。
...
- [x] Plate Live Preview 使用稳定 non-void inline；selection 进入时编辑原始语法，离开后立即恢复链接/嵌入投影。
- [x] `![[...]]` 只读嵌入支持 Markdown 全文、标题、block、Vault 图片和 PDF，内部链接可跳转，递归有循环与深度门禁。
...
- [x] 删除 Wiki cache 后能从 Vault 重建相同的索引语义；cache 不写入 Vault 或 catalog。
- [x] 20 个节点以内交互流畅。

后续增强 TODO：

- [x] Markdown、图片与 PDF 变更后防抖重建 wiki / Backlinks / Graph 索引（`scheduleWikiRebuild`，~900ms）；嵌入按目标路径精确刷新。
- [x] 源码编辑中的 `[[` 路径、alias、标题和 block 补全。
- [x] Plate 内联 wikilink 节点与规范链接序列化。
- [x] Markdown、图片与 PDF 的 `![[...]]` 只读嵌入及目标级刷新。
- [x] 显式标题重命名事务与编辑器入口。
- [x] Host 版本化 SQLite Wiki Metadata Cache 与冷重建诊断命令。
- [ ] 图谱 hover 时只高亮直接邻居。
