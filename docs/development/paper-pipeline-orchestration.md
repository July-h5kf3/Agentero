...
| 10 | enqueue 点收敛到 3 入口 + `reconcile` 扫描 + 删 `downloadAllMissingAssets` / `loadPaperRefsAuto` | 高 | 重复触发消失（依赖 9） | 大部分：投影(layout/parseRefs/parseBody/download)+取消收口+running 取消、layout/`enqueuePaperPdfParse` 委托、per-paper/vault-wide/open reconcile driver、`DownloadAssets` runner（`a9ac270d`）+ `downloadAllMissingAssets`/post-import 改 enqueue job（`25667536`/`9489ff1a`）、删 `maybeTriggerDeferredParse`（`5ea4a4b9`）与 `collectPapersNeedingAssetDownload`（`9489ff1a`）；余下删 `loadPaperRefsAuto`（需引用面板事件驱动 + ParseRefs reconcile） |
