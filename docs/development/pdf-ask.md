...
| **双击** | 双击词/短句，按词边界取上下文，弹出迷你问答卡 |
| **悬停停留** | 指针在某处静止超过阈值 \(T\)（建议默认 600–800 ms，可设置），弹出迷你问答卡（无选区时以指针页坐标为锚） |
| **框选截图** | 工具栏「框选」或 **⌥/Alt+拖拽**（PS 选框）；裁剪页 canvas → 附件；可拖控制点调框后发送。ACP 以 `ContentBlock::Image` 下发 |
| **键入提问** | 卡内输入问题并发送；可多轮；结束会话后收起；有图时可纯图提问 |
| **边注圆片** | 结束后在 PDF **右侧页边空白** 渲染小圆片（类似飞书文档评论角标） |
| **回访** | Hover 打开线程；移出对话框隐藏 |
...
| 阶段 | 交付 | 验收 |
|---|---|---|
| **M1 选区 + 弹层** | 划词出现 popover；可输入本地假回复 | 选区稳定、滚动不崩 |
| **M2 持久化** | JSON 读写；重开 PDF 圆片仍在 | 缩放后圆片仍对齐 quote |
| **M3 ACP 接入** | 真流式回答；多轮；结束写盘 | 与 Agent 面板共用 provider 配置 |
| **M4 双击 / 悬停** | 触发完善；设置项阈值 | 防误触可接受 |
