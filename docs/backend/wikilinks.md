...
3. `WikiIndex.rebuild` 优先校验应用 cache 下的版本化 SQLite snapshot；schema/parser/Vault identity 与 snapshot integrity hash 一致即加载，文件集合与 size+mtime stat 指纹完全一致时直接恢复，否则只重新解析指纹变化的 Markdown 文件（未变文件复用缓存的 documents 与 occurrences），再全量重跑链接解析。
