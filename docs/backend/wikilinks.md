...
- 标题右键菜单的“重命名当前小标题…”是独立事务：只对本地、可写、已保存且 selection 位于标题的 Markdown 启用。Host 以 `expectedContent + headingPath + headingLine` 确认标题身份，精确改写已解析到该标题或受影响后代的 Wikilink、嵌入、Vault-local Markdown link 与同文件 fragment；文件目标、alias、label、embed 标记和无关正文保持不变。
...
- 标题 fragment 没有稳定 UUID；手工修改 Markdown heading 后旧 fragment 仍会失效。需要同步入链时必须使用显式标题重命名命令。设计依据与缓存边界见 [`../research/wikilink-heading-reference-stability.md`](../research/wikilink-heading-reference-stability.md)。
...
- 唯一事实来源：Markdown 中的 `[[wikilink]]`。内存索引可从 Markdown 重建；版本化 SQLite snapshot 仅保存在应用 cache 目录，完整校验后用于 warm restore，删除后不改变语义。
...
| `wiki_rename_heading` | ✅ | `{ vaultPath, path, headingPath, headingLine, expectedContent, newText, dirtyPaths? }` → 显式标题改名与入链事务 |
...
| `graph_rebuild` | ✅ | 校验 warm snapshot；未命中时从 Vault 全量重建内存索引并 best-effort 覆盖 cache |
| `wiki_cache_rebuild` | ✅ | 内部诊断：删除当前 Vault snapshot 后从 Vault 全量重建 |
| `graph_get_graph` | ✅ | 全图或局部邻域 `{ nodes, edges }` |

索引更新：Markdown 保存、Vault 扫描、打开 Vault 后 `rebuild`；查询侧 `ensure_vault` 惰性重建。snapshot 位于 `agentero_cache_dir()/wiki/<vault-key>.sqlite`，保存完整目标文件指纹、documents 与 resolved occurrences；schema/parser/Vault identity、指纹或完整性哈希任一不匹配即删除并冷重建。cache 读写失败只记录 warning，不阻塞内存查询或 Markdown 事务。
...
1. Rust：`extract_wikilinks(md)` + `resolve` + **内存索引**；版本化 SQLite snapshot 仅用于通过完整指纹与完整性校验后的 warm restore。
2. Tauri：`graph_get_backlinks` / `graph_rebuild`（参数 `vaultPath` + `path`）。  
3. 前端：`src/lib/wiki/api.ts` + `BacklinksPanel`；Demo 模式纯前端索引。  
4. **文件变更防抖重建**（已落地）：Vault watcher 报告 Markdown、图片或 PDF 变更后，前端 `useVaultFileEvents.onWikiChange` → `scheduleWikiRebuild`（约 **900ms** 防抖）触发 rebuild，使 Backlinks / Graph 在外部或 Agent 写盘后保持新鲜；嵌入内容只按 watcher 本批次实际触及的目标路径失效。任一目标文件变化都会使 snapshot 失效并全量解析；仍非边级增量。
...
2. Plate 标题右键菜单提供受控“重命名当前小标题…”Dialog；成功后重载受影响的打开文档并刷新关系与嵌入投影。普通 heading 编辑不推断 rename。
...
| 风险 | 缓解 |
|---|---|
| 大 Vault 全量扫描慢 | warm restore 使用 Host SQLite 派生 snapshot；文件变化仍以全量重建优先正确性，边级增量另立工作 |
