...
- **设置窗口**：App 内居中浮层 dialog（`SettingsWindow` + `overlay-stack`；`⌘,` / 菜单 / 齿轮）。曾实现独立原生单例窗口（`settings_window_open` + `?window=settings`），Windows 第二 webview 白屏故改回浮层；该链路代码已删除（可从 git 历史找回）。保存经 `settings_set` → 广播 `settings:changed` 同步。
