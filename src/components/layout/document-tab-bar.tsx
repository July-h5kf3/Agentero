...
/** Browser-style document tab strip for the center pane. */
	...
	return (
		...
		>
			{tabs.map((tab) => {
				...
				const label =
					tab.kind === "library" ? t("sidebar:papersLibrary.title") : tab.title;
				return (
					<div
						key={tab.id}
						role="tab"
						...
						className={cn(
							"group my-1 flex min-w-0 max-w-[200px] cursor-default items-center gap-1.5 rounded-md px-2 text-xs outline-none",
					...
					>
						...
						<button
							...
							className={cn(
								"grid size-4 shrink-0 place-items-center rounded hover:bg-muted-foreground/20",
								dirty
