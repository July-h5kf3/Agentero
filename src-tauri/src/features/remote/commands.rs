...
) -> Result<ApiResult<crate::features::remote::agent_catalog::RemoteAgentScanResponse>, String> {
    let op = OpTimer::start_with(
        "remote_agent_scan",
        format!("session={}", trunc(&args.session_id, 40)),
    );
    match crate::features::remote::agent_catalog::scan_remote_agents(
        registry.inner(),
        &args.session_id,
    )
    .await
...
pub async fn remote_agent_probe(
    registry: State<'_, Arc<RemoteRegistry>>,
    agent_registry: State<'_, crate::features::agent::AgentRegistry>,
    args: RemoteAgentProbeArgs,
) -> Result<ApiResult<crate::features::agent::models::ProbeResult>, String> {
    let op = OpTimer::start_with(
        "remote_agent_probe",
        format!(
            "session={} template={}",
    ...
    match crate::features::remote::agent_catalog::probe_remote_template(
        registry.inner(),
        &args.session_id,
        &args.template_id,
        proxy_enabled,
